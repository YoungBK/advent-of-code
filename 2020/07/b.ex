defmodule AdventOfCode202007.B do
  def read_data() do
    File.read!("input.txt")
    |> String.trim()
    |> String.split(~r/[\n]/)
...
""" |> String.trim()
  ...
  def make_mapping(map, [key|rest]) do
    Enum.map(rest, &parse_info/1)
    |> Enum.reduce(map, &add_key_val(&2, key, &1))
  end
  ...
  def count_chains(contains, color, multiplier \\ 1) do
    case Map.get(contains, color) do
      [{}]  -> 0
      list ->
        Enum.reduce(list, 0, fn {c,m}, acc -> m * multiplier + count_chains(contains, c, m * multiplier) + acc end)
    end
  end
